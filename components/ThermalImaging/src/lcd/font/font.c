...
uint8_t* font_GetFontStruct(uint8_t FontID, uint8_t Char)
{
    if (FontID >= (sizeof(font_table_funcs) / sizeof(font_table_funcs[0]))) {
        return NULL; // 无效的FontID
    }
    if (font_table_funcs[FontID] == NULL) {
        return NULL; // 未定义的字体函数
    }
...
{
    if (pCharTable == NULL) return 12; // 默认宽度
...
{
    if (pCharTable == NULL) return 16; // 默认高度
    pCharTable++;
...
{
    if (pCharTable == NULL) return NULL;
