...
static uint8_t a_sht31_write(uint16_t reg)
{
    ...
    esp_err_t ret = i2c_master_write_slave_reg(I2C_NUM, sht31_iic_addr, cmd_buffer[0], cmd_buffer + 1, 1, 100 / portTICK_PERIOD_MS);
...
{
    esp_err_t ret = i2c_master_read_slave_reg_16bit(I2C_NUM, sht31_iic_addr, reg, data, len, 100 / portTICK_PERIOD_MS);
    return (ret == ESP_OK) ? 0 : 1;
}
...
{
    esp_err_t ret = i2c_master_read_slave(I2C_NUM, sht31_iic_addr, data, len, 100 / portTICK_PERIOD_MS);
    return (ret == ESP_OK) ? 0 : 1;
}
